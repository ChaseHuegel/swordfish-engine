...
public class NetController
{
    ...
    /// </summary>
    public int MaxSessions { get; set; } = NetControllerSettings.DEFAULT_MAX_SESSIONS;
    ...
    private void Initialize(NetControllerSettings settings)
    {
        ...
        try
        {
            ...
            {
                TargetTickRate = settings.TickRate > 0 ? settings.TickRate : NetControllerSettings.DEFAULT_TICK_RATE,
            };
    ...
    private void InitializeSessions()
    {
        if (Sessions != null)
        {
            ...
            {
                if (session.ID != NetSession.LOCAL_OR_UNASSIGNED)
                {
    ...
    private void OnReceive(IAsyncResult result)
    {
        ...
        var netEventArgs = new NetEventArgs
        {
            Packet = packet,
            EndPoint = endPoint,
        };
        ...
        try
        {
                ...
                && (!packetDefinition.Ordered || packet.Sequence >= currentSequence.Received))
            {
                ...
                if (packetDefinition.Reliable)
                {
                    ...
                    {
                        var ack = (AckPacket)deserializeData;
    ...
    private void UnregisterReliablePacket(IPEndPoint endPoint, int packetID, uint sequence)
    {
        ...
        {
            for (var i = 0; i < reliablePackets.Count; i++)
            {
