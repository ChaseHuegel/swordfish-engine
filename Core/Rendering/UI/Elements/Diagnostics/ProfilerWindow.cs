...
namespace Swordfish.Core.Rendering.UI.Elements.Diagnostics
{
    public class ProfilerWindow : Element
    {
        ...
        public override void OnShow()
        {
            ...
            Profiler.Collect(ProfilerType.PHYSICS, Engine.Physics.Thread.DeltaTime, ImGui.IsWindowHovered(), out highest, out lowest, out average, out profile);
            Present("T - Physics", 0f, 16f, profile, highest, lowest, average);

            //  Profile and present ECS
            Profiler.Collect(ProfilerType.ECS, Engine.ECS.Thread.DeltaTime, ImGui.IsWindowHovered(), out highest, out lowest, out average, out profile);
            Present("T - ECS", 0f, 16f, profile, highest, lowest, average);

            //  Profile and present Main
            Profiler.Collect(ProfilerType.MAIN, Engine.MainWindow.DeltaTime, ImGui.IsWindowHovered(), out highest, out lowest, out average, out profile);
